    ...
    static class FishingSpotExtensions
    {
        ...
        static FishingSpotExtensions()
        {
            _pageIdSourceOfCBH =
                ...
                .ToDictionary(item => new GameDataObjectId(GameDataObjectCategory.FishingSpot, item.spotId), item => item.pageId);
        }

        public static string GetCBHLink(this FishingSpot spot)
        {
            string pageId;
            if (!_pageIdSourceOfCBH.TryGetValue(spot.Id, out pageId))
                return null;
            var format = Translate.Instance[new TranslationTextId(TranslationCategory.Url, "CBH.SpotPage")];
