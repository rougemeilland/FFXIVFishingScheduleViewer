        ...
        private IEnumerable<RateInfo> ParseRate(string doc)
        {
            ...
            return
                ...
                .Select((m, index) =>
                {
                    return new RateInfo
                    {
                        FishRawId = m.Groups["fish"].Value.Trim(),
                        BaitRawId = m.Groups["bait"].Value.Trim(),
                        Order = index,
                        Numerator = int.Parse(m.Groups["numerator"].Value),
                        Denominator = int.Parse(m.Groups["denominator"].Value),
                    };
                })
                .ToList();
        }
